    ...
    if(err) {
      ...
      self.provider.validateNotReplay(requestParameters.oauth_token, requestParameters.oauth_timestamp, requestParameters.oauth_nonce, function(err, result) {
        if(err) {
          ...
          self.provider.userByConsumerKey(token.consumer_key, function(err, user) {
            if(user.consumer_key == null || user.secret == null) { callback(new errors.OAuthProviderError("provider: userByConsumerKey must return a object with fields [token, secret]"), null); return;}
...
OAuthServices.prototype.requestToken = function(method, protocol, url, path, headers, parameters, callback) { 
  ...
  this.provider.userByConsumerKey(requestParameters['oauth_consumer_key'], function(err, user) {
    if(err) {
