      ...
      self.provider.validateNotReplay(requestParameters.oauth_token, requestParameters.oauth_timestamp, requestParameters.oauth_nonce, function(err, result) {
        if(err) {
          ...
          self.provider.applicationByConsumerKey(token.consumer_key, function(err, user) {
            ...
            sys.puts("==========================================================================================================");
            sys.puts("= calculatedSignature: " + calculatedSignature);
            sys.puts("= method: " + method);
            sys.puts("= protocol: " + protocol);
            sys.puts("= path: " + path);
            sys.puts("= requestParameters: " + sys.inspect(requestParameters));
            sys.puts("= tokenObject.token_secret: " + tokenObject.token_secret);
            sys.puts("= user.secret: " + user.secret);
